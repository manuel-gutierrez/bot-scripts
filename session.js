...
function is_session_expired() {
	...
	if (inputs.session_timestamp) {
	 last_session_timestamp = inputs.session_timestamp;
	 diff =   now_timestamp - last_session_timestamp;
	 diff_in_hrs = Math.floor((diff % 86400000) / 3600000);
	 if (diff_in_hrs < 1) { return false } else {return true}
	} else {
	  return(true); 
	}
}
...
{{main_flow_catch_all.attributes__session_timestamp}}